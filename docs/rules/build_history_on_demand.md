...
**Saída:** `docs/out/historico_<assunto>.md` (descartável; regenerável a qualquer momento).
...
| 3   | atual: router + `docs/spec/` **+** `docs/rules`/`docs/templates`                                                                                        | disco                         |
...
1. `--follow` **exige um só** pathspec e **para** quando o ficheiro é novo: em `docs/spec/booking.md`
...
Pipeline + `php tools/health-check.php` → **TUDO OK** (`docs/README.md`). Sem BOM, CRLF.
