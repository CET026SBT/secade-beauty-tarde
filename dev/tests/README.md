...
```bash
php dev/tests/js_syntax_check.php   # SINTAXE JS: OK                        (sem dependências)
php dev/tests/functional_test.php   # 105 pass, 0 fail                     (requer MySQL)
php dev/tests/http_test.php         # 119 pass, 0 fail                     (requer Apache + MySQL)
php dev/tests/asset_test.php        #  65 pass, 0 fail                     (requer Apache)
```
...
- **Testes manuais** (fluxos completos por interface, navegação mobile, responsividade): o guia é
  **gerado sob demanda** (`dev/docs/rules/`) — não é ficheiro mantido.
...
> As suites são independentes de `dev/tools/` e da documentação: um `health-check` verde **não** valida
> testes, e vice-versa.
