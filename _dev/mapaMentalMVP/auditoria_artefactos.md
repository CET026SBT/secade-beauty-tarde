...
especificação passar a **router** (`especificacao_mvp.md`) + **conteúdo por domínio** (`_dev/docs/spec/`).
...
`admin-employee-list`/`-save`/`-toggle`; a **§19.2** (`_dev/docs/spec/data-api.md`) tem os endpoints do
...
**Prova:** `_dev/docs/spec/requirements.md` — §4.6 termina em **RF-74** ✔ · §5.1 termina em **RN-29** ✔ ·
**D-11** é o último `D-nn` (`_dev/docs/spec/core.md` §3) ✔. Os limites estão certos; **RF-75…RF-79 ficavam
...
espelhava as duas: **§12.2** (`_dev/docs/spec/booking.md`, custo) e **§3.1** (`_dev/docs/spec/core.md`, referência
...
papel provisório que os 50 € desempenhavam. Registado em **§25.3** (`_dev/docs/spec/backlog.md`), com as
...
| `admin-dashboard-summary`, `admin-accounting-balance`,        | (`_dev/docs/spec/data-api.md`). F-04 aplicado: `employee-*`    |
...
|                                                               | conteúdo em **9** ficheiros de `_dev/docs/spec/` — `md-verify` |
|                                                               | **TUDO OK (22 ficheiros)**. Nenhuma âncora `§NN` partir        |
| Todas as secções citadas existem                              | §4.6 · §5.1 · §10.6 · §12.4 · §14.1/2 · §15.3 · §17.6/8 ·      |
| (todas em `_dev/docs/spec/`, mapa `§ → ficheiro` no router)   | §18.x · §19.2/3/5 · §22.1/2/3 · §23.4 · §24.2/6 ·              |
...
**Ficheiro:** `_dev/mapaMentalMVP/auditoria_artefactos.md` · **branch:** `agent-workspace` ·
