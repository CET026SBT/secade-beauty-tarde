...
> chaves legadas (PT, snake_case) e o validator que a verifica (`_dev/tests/asset_test.php`).
...
- `_dev/tools/` — utilitários de manutenção dev-only (encoding, formatação/validação de `.md`,
  ...
  `_dev/tools/README.md`; onde a pasta vive está em `.clinerules` §4.
...
> 🔗 **Fonte única: `_dev/tools/README.md`** — catálogo das ferramentas, opções, convenções comuns
